...
Columns with more than 40% missing values were dropped (58 columns). This threshold was just arbitrarily chosen — the rationale behind this decision was that imputing more than 40% of a column's values introduces more noise than signal.
...
1. **Regulatory interpretability (SR 11-7):** Each feature has an explicit coefficient. The contribution of every variable to every score is transparent and auditable.
...
**Calibration:** Mean predicted probability on test = 0.1805. Actual default rate on test = 0.2242. The model systematically underpredicts default probability on the test set. Would like to attribute this dip in prediction to macroeconomic shifts (hopefully).
